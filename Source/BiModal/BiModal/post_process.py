...
def get_accuracy(file, run_length_enable = True, run_length_clip_skip = 0, threshold = 0.5, tolerance = 0.5, weights = None, counts = None):
	f = open(file, "r")
	lines = f.readlines()
	...
	for i in range(len(conf_mat)):
		...
		if pr != 0 and re != 0:
			...
			f1 = 0
		f1_1.append(weights[i] * f1)
		wpr.append(weights[i]*pr)
		wr.append(weights[i]*re)
		print("%d Precision:%.2f Recall:%.2f F-Measure:%.2f Weight:%.2f W1F:%.2f"%(i, pr, re, f1, weights[i], weights[i] * f1))
	f11 = np.sum(f1_1)
	f11 = float(f11)/np.sum(weights.values())
	weightedPrecision = np.sum(wpr)
	weightedPrecision = float(weightedPrecision)/np.sum(weights.values())
	weightedRecall = np.sum(wr)
	weightedRecall = float(weightedRecall)/np.sum(weights.values())
	if weightedPrecision != 0 and weightedRecall != 0:
		f12 = float(2 * weightedPrecision * weightedRecall)/(weightedPrecision + weightedRecall)
	else:
		f12 = 0
	print("======F1 Calculation1========")
	print("%.4f"%(f11))
	print("======F1 Calculation2========")
	print("%.4f"%(f12))
	return f12

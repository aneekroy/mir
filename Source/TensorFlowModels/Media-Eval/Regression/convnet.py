...
tf.app.flags.DEFINE_integer('batch_size', 6290,
                            """Number of images to process in a batch.""")
tf.app.flags.DEFINE_string('data_dir', '/home/soms/EmotionMusic/MediaEval',
...
#IMAGE_SIZE = 32
NUM_CLASSES = 1
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 30950
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 6290
...
def inference(images, eval = False):
  ...
  with tf.variable_scope('conv1') as scope:
    try:
      kernel = _variable_with_weight_decay('weights', shape=[4, 4, 1, 64], stddev=5e-2, wd=0.0)
    except ValueError:
  ...
  # pool1
  pool1 = tf.nn.max_pool(drop1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool1')
  ...
  with tf.variable_scope('conv2') as scope:
    try:
      kernel = _variable_with_weight_decay('weights', shape=[4, 4, 64, 64], stddev=5e-2, wd=0.0)
    except ValueError:
      scope.reuse_variables()
      kernel = _variable_with_weight_decay('weights', shape=[4, 4, 64, 64], stddev=5e-2, wd=0.0)
  ...
  # pool2
  #pool2 = tf.nn.max_pool(drop2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool2')
  ...
  # drop3
  drop3 = tf.nn.dropout(conv3, keep_prob = keep3, name='drop2')
  _activation_summary(drop3)

  # pool3
  pool3 = tf.nn.max_pool(drop3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool3')
  ...
  with tf.variable_scope('local4') as scope:
    ...
    try:
      weights = _variable_with_weight_decay('weights', shape=[dim, 512], stddev=0.05, wd=0.0)
  ...
  with tf.variable_scope('local5') as scope:
    try:
      weights = _variable_with_weight_decay('weights', shape=[512, 256], stddev=0.05, wd=0.0)
  ...
  with tf.variable_scope('local6') as scope:
    try:
      weights = _variable_with_weight_decay('weights', shape=[256, 64], stddev=0.05, wd=0.0)
      biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
    except ValueError:
      scope.reuse_variables()
      weights = _variable_with_weight_decay('weights', shape=[256, 64], stddev=0.05, wd=0.0)
      biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
    local6 = tf.nn.relu(tf.matmul(drop5, weights) + biases, name=scope.name)
    _activation_summary(local6)

  # drop6
  drop6 = tf.nn.dropout(local6, keep_prob = keep6, name='drop5')
  _activation_summary(drop6)
  ...
  with tf.variable_scope('softmax_linear') as scope:
    try:
      ...
      weights = _variable_with_weight_decay('weights', [64, NUM_CLASSES], stddev=1/64.0, wd=0.0)
      biases = _variable_on_cpu('biases', [NUM_CLASSES], tf.constant_initializer(0.0))
    softmax_linear = tf.add(tf.matmul(drop6, weights), biases, name=scope.name)
    _activation_summary(softmax_linear)

  return softmax_linear

def loss(logits, labels):
  #shape = int(labels.get_shape()[0])
  labels = tf.cast(labels, tf.float32)
  #labels = tf.reshape(labels, [shape, 1])
  logits = tf.squeeze(logits)
  mse_mean = tf.reduce_mean(tf.square(logits - labels), name = 'mse_mean')
  tf.add_to_collection('losses', mse_mean)
...
def train(total_loss, global_step):
  ...
  with tf.variable_scope("conv1"):
    ...
    w = tf.transpose(weights, [3, 0 , 1, 2]) 
    tf.image_summary("conv1/features_isolated", w, max_images = 64)
  ...
  with tf.variable_scope("conv2"):
    ...
    #tf.image_summary("conv2/features_isolated", weights, max_images = 5)

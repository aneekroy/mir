...
ALPHA_MAP = {0:"H", 1:"S", 2:"T", 3:"F/A"}

FLAGS = tf.app.flags.FLAGS

tf.app.flags.DEFINE_string('eval_dir', '/home/soms/EmotionMusic/Model2_SongWise/Evaluation_Logs',
                           """Directory where to write event logs.""")
tf.app.flags.DEFINE_string('eval_data', 'test',
                           """Either 'test' or 'train_eval'.""")
tf.app.flags.DEFINE_string('checkpoint_dir', '/home/soms/EmotionMusic/Model2_SongWise/Training_Logs',
                           """Directory where to read model checkpoints.""")
tf.app.flags.DEFINE_integer('eval_interval_secs', 1.5 * 60,
                            """How often to run the eval.""")
tf.app.flags.DEFINE_integer('num_examples', 74,
...
def eval_once(sess, saver, logits, labels, filename, predictionfile, sequencefile):
  ...
  predicted_max = 0
  for i in range(NUM_CLASSES):
    if preds[i] > preds[predicted_max]:
      predicted_max = i
  prec_song = 0
  if predicted_max == labels[0]:
    prec_song = 1
  tot_clips = len(probs)
  prec_clips = 0
  for i in range(len(probs)):
...
def evaluate(ind = None):
  ...
  testdir = "/home/soms/EmotionMusic/Model2_SongWise/Data/Test"
  index = ind if ind is not None else 0
  while True:
    with tf.Graph().as_default() as g:
      predictionfile = open("/home/soms/EmotionMusic/Model2_SongWise/Outputs/output_" + str(index) + ".txt", "w")
      sequencefile = open("/home/soms/EmotionMusic/Model2_SongWise/Outputs/output_sequence_" + str(index) + ".txt", "w")
...
if __name__ == '__main__':
  try:
    ...
    print("Usage: python model2_eval_classwise.py index")
    sys.exit(0)
  evaluate(ind)
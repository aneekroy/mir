...
tf.app.flags.DEFINE_string('train_dir', '/home/soms/EmotionMusic/MediaEval/Training_Logs',
...
def train():
  with tf.Graph().as_default():
    ...
    print("Summary op created...")

    # Build an initialization operation to run below.
    init = tf.initialize_all_variables()
    print("Init op created...")
    ...
    stepfile = "/home/soms/EmotionMusic/MediaEval/stepfile.txt" 
    ...
    while step < FLAGS.max_steps:
        #print(images.eval(session = sess).shape)
      	...
      	_, loss_value, squared_error, mean_error = sess.run([train_op, loss, R_y, R_e])
        ...
        signal.signal(signal.SIGINT, signal_handler)
      	...
      	if step % 10 == 0:
        	num_examples_per_step = 8
        	examples_per_sec = num_examples_per_step / duration
        	sec_per_batch = float(duration)
          	format_str = ('%s: step %d, loss = %.2f r-squared = %.2f (%.1f examples/sec; %.3f ''sec/batch)')
	  	print (format_str % (datetime.now(), step, loss_value, R_squared , examples_per_sec, sec_per_batch))

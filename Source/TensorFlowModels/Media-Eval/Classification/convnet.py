...
tf.app.flags.DEFINE_integer('batch_size', 375,
                            """Number of images to process in a batch.""")
tf.app.flags.DEFINE_string('data_dir', '/home/soms/EmotionMusic/MediaEval_Classification',
...
#IMAGE_SIZE = 32
NUM_CLASSES = 4
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 46425
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 9375
...
def inputs(eval_data):
  ...
  batch_size = FLAGS.batch_size if eval_data else 25
...
def inference(images, eval = False):
  ...
  with tf.variable_scope('conv1') as scope:
    try:
      kernel = _variable_with_weight_decay('weights', shape=[3, 3, 1, 64], stddev=5e-2, wd=0.0)
    except ValueError:
  ...
  print("Pool 3 shape", pool3.get_shape())
  ...
  with tf.variable_scope('local4') as scope:
    ...
    try:
      weights = _variable_with_weight_decay('weights', shape=[dim, 512], stddev=0.04, wd=0.1)
  ...
  # drop4
  drop4 = tf.nn.dropout(local4, keep_prob = keep_prob, name='drop4')
  ...
  print("Local 5 shape", local5.get_shape())

  # drop5
  drop5 = tf.nn.dropout(local5, keep_prob = keep_prob, name='drop5')

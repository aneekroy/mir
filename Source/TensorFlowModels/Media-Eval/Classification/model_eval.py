...
tf.app.flags.DEFINE_integer('num_examples', 9375,
    ...
    try:
      ...
      for index in range(1):
        ...
        while step < num_iter and not coord.should_stop():
          ...
          print("Step:%d"%step)
          print("Step %d Loss %.2f Accuracy@1 %.2f"%(step, loss_val, float(np.sum(predictions)) / FLAGS.batch_size))
          step += 1
        # Compute precision @ 1.
        precision = true_count / (FLAGS.batch_size * step)
      ...
      rec = {}
      fm = {}
      confusion_matrix = np.sum(np.asarray(cf), 0)
      #confusion_matrix = np.asarray(cf)
      print("Confusion Matrix")
      print(confusion_matrix)
      print("Sum CF:%.2f"%np.sum(confusion_matrix))
      ...
      print("\nAverage accuracy %.3f Min accuracy %.3f Max accuracy %.3f"%(mean_precision, m, M))
      ...
      summary.value.add(tag='Accuracy @ 1', simple_value=mean_precision)

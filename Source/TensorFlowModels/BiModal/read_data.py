	...
	for _, _, files in os.walk(train_path):
		for file in files:
			...
			try:
				...
				train_x.append(np.asarray(Image.open(os.path.join(train_path, file)).convert("L")).T)
	...
	for _, _, files in os.walk(test_path):
		for file in files:
			...
			try:
				...
				test_x.append(np.asarray(Image.open(os.path.join(test_path, file)).convert("L")).T)
...
if __name__ == "__main__":
	...
	train_x, train_y, test_x, test_y, _, _= load_dataset("/home/soms/EmotionMusic/bimodal/Features", "/home/soms/EmotionMusic/MER_bimodal_dataset/annotations.csv")

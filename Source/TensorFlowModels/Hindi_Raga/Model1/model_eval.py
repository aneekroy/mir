...
tf.app.flags.DEFINE_string('eval_dir', '/home/soms/EmotionMusic/ModelRaga/Evaluation_Logs',
                           """Directory where to write event logs.""")
tf.app.flags.DEFINE_string('eval_data', 'test',
                           """Either 'test' or 'train_eval'.""")
tf.app.flags.DEFINE_string('checkpoint_dir', '/home/soms/EmotionMusic/ModelRaga/Training_Logs',
                           """Directory where to read model checkpoints.""")
tf.app.flags.DEFINE_integer('eval_interval_secs', 1.5 * 60,
                            """How often to run the eval.""")
tf.app.flags.DEFINE_integer('num_examples', 1635,
                            """Number of examples to run.""")
tf.app.flags.DEFINE_boolean('run_once', False,
                         """Whether to run eval only once.""")


def eval_once(sess, saver, summary_writer, logits, labels, top_k_op, conf_mat, loss, summary_op):
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    images, labels = convlstm.inputs(eval_data = True)

    # Build a Graph that computes the logits predictions from the
    # inference model.
    logits = convlstm.inference(images, eval = True)
    loss = convlstm.loss(logits, labels)
    ...
    variable_averages = tf.train.ExponentialMovingAverage(convlstm.MOVING_AVERAGE_DECAY)
    ...
    while True:
      with tf.Session() as sess:   
        eval_once(sess, saver, summary_writer, logits, labels, top_k_op, conf_mat, loss, summary_op)
        if FLAGS.run_once:
          break

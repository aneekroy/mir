			...
			for _,_, files in os.walk(labdir):	
				...
				for file in trainset:
					...
					for image_segment in image:
						...
						plt.imsave(os.path.join(featuredir + "/Train", filename + "_" + str(index) + ".png"), image_segment.T, cmap = "gray")
						f.write(os.path.join(featuredir + "/Train", filename + "_" + str(index) + ".png") + ":" + str(classindex)+"\n")
						index += 1
				for file in testset:
					fileindex = int(file[:file.find(".wav")])
					classindex = math.trunc((math.ceil(fileindex/30.)))
					...
					for image_segment in image:
						image_segment = image_segment[image_segment.shape[0] - 57 : ]
						#print(image_segment.T.shape)
						plt.imsave(os.path.join(featuredir + "/Test", filename + "_" + str(index) + ".png"), image_segment.T, cmap = "gray")
						f.write(os.path.join(featuredir + "/Test", filename + "_" + str(index) + ".png") + ":" + str(classindex)+"\n")
						index += 1

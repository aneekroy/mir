...
from keras.layers import Dense, Dropout, Flatten, Reshape, Bidirectional, TimeDistributedDense, Merge, Activation, BatchNormalization, GlobalAveragePooling2D
...
def show_results(model, testrepo, history = None):
	...
	# Find the positions of the different segments of a single test excerpt. Each such segment is
	# represented by the pair(segment_index, actual_index_in_list)	
	segment_indices = {}
	for index in range(len(testrepo)):
		file = testrepo[index]
		filename = file[file.find("_") + 1 : file.rfind("_")]
		fileindex = file[file.rfind("_") + 1 : file.find(".")]
	...
	f = open("/home/soms/EmotionMusic/Model4/sequence.txt", "w")
	
	whole_set_accuracy = 0.0
	LAB_MAP = {0:"H", 1:"S", 2:"T", 3:"F/A"}
...
def class_weights(lab_dict):
	...
	for key in lab_dict.keys():
		cw[int(key)] = float(M) / lab_dict[key]
	maxcw = max(cw.values())
	for key in cw:
		cw[key] = float(cw[key]) / maxcw
		print(str(key) + ":" + str(cw[key])) 
	return cw

seed = 7
np.random.seed(7)
DATA_PATH = "/home/soms/EmotionMusic/Segmented_Features"

x_train, y_train, x_test, y_test, testrepo, train_cnt= load_dataset(DATA_PATH)
...
# ConvPool3
model.add(Convolution2D(128, 3, 3, border_mode = 'same', activation = 'relu', W_constraint = maxnorm(3), W_regularizer = l2(0.01)))
model.add(MaxPooling2D(pool_size = (2, 2), strides = (2, 2)))
...
# Dense1
model.add(Dense(512, activation = 'relu', W_regularizer = l2(0.05)))

# Dropout1
model.add(Dropout(0.5))

# Dense2
model.add(Dense(256, activation = 'relu', W_regularizer = l2(0.05)))
...
# Checkpointing
filepath = "/home/soms/EmotionMusic/Model4/Saved_Weights/weights_best.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor = 'val_acc', verbose = 1, save_best_only = True, mode = 'max')
callbacks_list = [checkpoint]

'''
# Tensorboard
tensorboardfile = "/home/soms/EmotionMusic/Model4/Logs"
...
history = model.fit(x_train, y_train, validation_data=(x_test, y_test), nb_epoch = epochs, callbacks = callbacks_list, batch_size = 16)

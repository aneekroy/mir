...
def convert_to_tfrecord(recordsdir, tfdir):
	...
	validcount = 0
	testcount = 0
	#trainlogger = open("/home/soms/EmotionMusic/Model2/log-train.txt", "w")
	...
	for _, classes, files in os.walk(recordsdir):
		index = -1
		for label in classes:
			...
			for _, _, files in os.walk(os.path.join(recordsdir, label)):
				size = len(files)
				trainset = files[: int(TRAIN_PERCENT * size)]
				validset = files[int(TRAIN_PERCENT * size) : int((TRAIN_PERCENT + VALID_PERCENT) * size)]
				testset = files[int((TRAIN_PERCENT + VALID_PERCENT) * size) :]
				traincount += len(trainset)
				validcount += len(validset)
...
if __name__ == "__main__":
	convert_to_tfrecord("/home/soms/EmotionMusic/Hindi_Spec", "/home/soms/EmotionMusic/ModelRaga/Data")
